...
ErrorCode MetadataSet::RefineMetadata(std::vector<SizeType> &indices, std::shared_ptr<MetadataSet> &p_newMetadata,
                                      std::uint64_t p_blockSize, std::uint64_t p_capacity,
                                      std::uint64_t p_metaSize) const
{
...
ErrorCode MetadataSet::RefineMetadata(std::vector<SizeType> &indices, std::shared_ptr<Helper::DiskIO> p_metaOut,
                                      std::shared_ptr<Helper::DiskIO> p_metaIndexOut) const
{
...
ErrorCode MetadataSet::RefineMetadata(std::vector<SizeType> &indices, const std::string &p_metaFile,
                                      const std::string &p_metaindexFile) const
{
    {
        std::shared_ptr<Helper::DiskIO> ptrMeta = f_createIO(), ptrMetaIndex = f_createIO();
        if (ptrMeta == nullptr || ptrMetaIndex == nullptr ||
            !ptrMeta->Initialize((p_metaFile + "_tmp").c_str(), std::ios::binary | std::ios::out) ||
            !ptrMetaIndex->Initialize((p_metaindexFile + "_tmp").c_str(), std::ios::binary | std::ios::out))
...
void MetadataSet::AddBatch(MetadataSet &data)
{
...
bool MetadataSet::GetMetadataOffsets(const std::uint8_t *p_meta, const std::uint64_t p_metaLength,
                                     std::uint64_t *p_offsets, std::uint64_t p_offsetLength, char p_delimiter)
{
                                 ...
                                 std::uint64_t p_blockSize, std::uint64_t p_capacity, std::uint64_t p_metaSize)
{
    ...
    if (m_fp == nullptr || fpidx == nullptr || !m_fp->Initialize(p_metafile.c_str(), std::ios::binary | std::ios::in) ||
        !fpidx->Initialize(p_metaindexfile.c_str(), std::ios::binary | std::ios::in))
    {
...
ByteArray FileMetadataSet::GetMetadata(SizeType p_vectorID) const
{
...
ByteArray FileMetadataSet::GetMetadataCopy(SizeType p_vectorID) const
{
    std::unique_lock<std::shared_timed_mutex> lock(*static_cast<std::shared_timed_mutex *>(m_lock.get()));
    std::uint64_t startoff = m_offsets[p_vectorID];
    std::uint64_t bytes = m_offsets[p_vectorID + 1] - startoff;
...
std::pair<std::uint64_t, std::uint64_t> FileMetadataSet::BufferSize() const
{
    std::shared_lock<std::shared_timed_mutex> lock(*static_cast<std::shared_timed_mutex *>(m_lock.get()));
    return std::make_pair(m_offsets.back(), sizeof(SizeType) + sizeof(std::uint64_t) * m_offsets.size());
}
...
ErrorCode FileMetadataSet::SaveMetadata(std::shared_ptr<Helper::DiskIO> p_metaOut,
                                        std::shared_ptr<Helper::DiskIO> p_metaIndexOut)
{
    std::shared_lock<std::shared_timed_mutex> lock(*static_cast<std::shared_timed_mutex *>(m_lock.get()));
...
ErrorCode FileMetadataSet::SaveMetadata(const std::string &p_metaFile, const std::string &p_metaindexFile)
{
    {
        std::shared_ptr<Helper::DiskIO> metaOut = f_createIO(), metaIndexOut = f_createIO();
        if (metaOut == nullptr || metaIndexOut == nullptr ||
            !metaOut->Initialize((p_metaFile + "_tmp").c_str(), std::ios::binary | std::ios::out) ||
            !metaIndexOut->Initialize((p_metaindexFile + "_tmp").c_str(), std::ios::binary | std::ios::out))
            return ErrorCode::FailedCreateFile;
...
MemMetadataSet::MemMetadataSet(std::uint64_t p_blockSize, std::uint64_t p_capacity, std::uint64_t p_metaSize)
    : m_count(0), m_metadataHolder(ByteArray::c_empty)
{
                               ...
                               std::uint64_t p_blockSize, std::uint64_t p_capacity, std::uint64_t p_metaSize)
{
    std::shared_ptr<Helper::DiskIO> ptrMeta = f_createIO(), ptrMetaIndex = f_createIO();
    if (ptrMeta == nullptr || ptrMetaIndex == nullptr ||
        !ptrMeta->Initialize(p_metafile.c_str(), std::ios::binary | std::ios::in) ||
        !ptrMetaIndex->Initialize(p_metaindexfile.c_str(), std::ios::binary | std::ios::in))
    {
        SPTAGLIB_LOG(Helper::LogLevel::LL_Error, "ERROR: Cannot open meta files %s or %s!\n", p_metafile.c_str(),
                     p_metaindexfile.c_str());
...
MemMetadataSet::MemMetadataSet(ByteArray p_metadata, ByteArray p_offsets, SizeType p_count, std::uint64_t p_blockSize,
                               std::uint64_t p_capacity, std::uint64_t p_metaSize)
    : m_count(p_count), m_metadataHolder(std::move(p_metadata))
{
    m_pOffsets.reset(new MetadataOffsets, std::default_delete<MetadataOffsets>());
    auto &m_offsets = *static_cast<MetadataOffsets *>(m_pOffsets.get());
...
ByteArray MemMetadataSet::GetMetadata(SizeType p_vectorID) const
{
    auto &m_offsets = *static_cast<MetadataOffsets *>(m_pOffsets.get());
    std::uint64_t startoff = m_offsets[p_vectorID];
    std::uint64_t bytes = m_offsets[p_vectorID + 1] - startoff;
...
ByteArray MemMetadataSet::GetMetadataCopy(SizeType p_vectorID) const
{
    auto &m_offsets = *static_cast<MetadataOffsets *>(m_pOffsets.get());
    std::uint64_t startoff = m_offsets[p_vectorID];
    std::uint64_t bytes = m_offsets[p_vectorID + 1] - startoff;
...
SizeType MemMetadataSet::Count() const
{
    auto &m_offsets = *static_cast<MetadataOffsets *>(m_pOffsets.get());
    return static_cast<SizeType>(m_offsets.size() - 1);
}
...
std::pair<std::uint64_t, std::uint64_t> MemMetadataSet::BufferSize() const
{
    auto &m_offsets = *static_cast<MetadataOffsets *>(m_pOffsets.get());
    auto n = m_offsets.size();
    return std::make_pair(m_offsets[n - 1], sizeof(SizeType) + sizeof(std::uint64_t) * n);
}
...
ErrorCode MemMetadataSet::SaveMetadata(std::shared_ptr<Helper::DiskIO> p_metaOut,
                                       std::shared_ptr<Helper::DiskIO> p_metaIndexOut)
{
    auto &m_offsets = *static_cast<MetadataOffsets *>(m_pOffsets.get());
    SizeType count = Count();
...
ErrorCode MemMetadataSet::SaveMetadata(const std::string &p_metaFile, const std::string &p_metaindexFile)
{
    {
        std::shared_ptr<Helper::DiskIO> metaOut = f_createIO(), metaIndexOut = f_createIO();
        if (metaOut == nullptr || metaIndexOut == nullptr ||
