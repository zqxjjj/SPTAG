...
\
    ...
    if (p_space.m_iNumOfContinuousNoBetterPropagation > m_iThresholdOfNumberOfContinuousNoBetterPropagation) { \
        if (p_space.m_iNumberOfTreeCheckedLeaves <= p_space.m_iNumberOfCheckedLeaves / 10) { \
            m_pTrees.SearchTrees<T,Q>(m_pSamples, m_fComputeDistance, p_query, p_space, m_iNumberOfOtherDynamicPivots +
p_space.m_iNumberOfCheckedLeaves); \
...
#define DefineVectorValueType(Name, Type)                                                                              \
    case VectorValueType::Name:                                                                                        \
        ...
        m_pTrees.SearchTrees<T, Type>(m_pSamples, m_fComputeDistance, *p_results, *workSpace,                          \
...
#undef DefineVectorValueType
    ...
    else
    {
        ...
        m_pTrees.SearchTrees<T, T>(m_pSamples, m_fComputeDistance, *p_results, *workSpace,
                                   m_iNumberOfInitialDynamicPivots);
    }
...
#define DefineKDTParameter(VarName, VarType, DefaultValue, RepresentStr) ptr->VarName = VarName;
...
#define DefineVectorValueType(Name, Type) template class SPTAG::KDT::Index<Type>;
