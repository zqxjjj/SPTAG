                        ...
                        if (index < currentPostingNum)
                        {
                            ...
                            else
                            {
                                ...
                                if ((ret = db->Get(index, &postingList, MaxTimeout, &(workSpace.m_diskRequests))) !=
                                    ErrorCode::Success)
                                {
                                    SPTAGLIB_LOG(Helper::LogLevel::LL_Error, "RefineIndex failed to get posting %d\n", index);
                                    finalcode = ret;
                                    return;
                                }
            ...
            if (clear) {
                ...
                if (p_exWorkSpace->g_spaceCount.load() >=
                    max(m_opt->m_ioThreads,
                        (2 * max(m_opt->m_searchThreadNum, m_opt->m_iSSDNumberOfThreads) + m_opt->m_insertThreadNum +
