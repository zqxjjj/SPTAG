            ...
            std::shared_ptr<VectorSet> &truth)
{
    ...
    for (SizeType i = 0; i < queryset->Count(); i++)
    {
        ...
        for (int j = 0; j < truthDimension; j++)
        {
            ...
            float truthdist =
                vecIndex->ComputeDistance(queryset->GetVector(i), vecIndex->GetSample(truthmeta, deleted));
            for (int l = 0; l < k; l++)
            {
...
template <typename T>
void PerfAdd(IndexAlgoType algo, std::string distCalcMethod, std::shared_ptr<VectorSet> &vec,
             ...
             std::shared_ptr<VectorSet> &truth, std::string out)
{
    ...
    for (SizeType i = 0; i < vec->Count(); i++)
    {
        ...
        std::shared_ptr<MetadataSet> metaset(
            new MemMetadataSet(metaarr, ByteArray((std::uint8_t *)offset, 2 * sizeof(std::uint64_t), false), 1));
...
template <typename T>
void PerfBuild(IndexAlgoType algo, std::string distCalcMethod, std::shared_ptr<VectorSet> &vec,
               std::shared_ptr<MetadataSet> &meta, std::shared_ptr<VectorSet> &queryset, int k,
               std::shared_ptr<VectorSet> &truth, std::string out)
{
...
template <typename T>
void GenerateData(std::shared_ptr<VectorSet> &vecset, std::shared_ptr<MetadataSet> &metaset,
                  std::shared_ptr<VectorSet> &queryset, std::shared_ptr<VectorSet> &truth, std::string distCalcMethod,
                  int k)
{
        ...
        fileexists("perftest_query.bin"))
    {
        ...
        metaset.reset(new MemMetadataSet("perftest_meta.bin", "perftest_metaidx.bin", vecset->Count() * 2,
                                         vecset->Count() * 2, 10));
...
template <typename T> void PTest(IndexAlgoType algo, std::string distCalcMethod)
{
